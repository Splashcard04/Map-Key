...
export class noteMod {
   ...
   */
  noteLine(
    position: [number, number] = [1.5, 0],
    jumpTime: [number, number] = [0.35, 0.45],
  ) {
  ...
  noteTimeSlow(
    slowPoint = 0.1,
    slowForce: [easeBase, easeBase] = ["Sine", "Sine"],
    offset = 2,
    specialEase?: [
      "Bounce" | "Back" | "Elastic",
      "Bounce" | "Back" | "Elastic",
    ...
    if (slowPoint >= 0.5) {
      MKLog(
        `A resume point of ${slowPoint} is past the recommended maximum of 0.5, unexpected behaviour may occur...`,
        "Warning",
      );
    }
  }
  ...
  noteBeatPulse(pulseSize = 1.5) {
    allBetween(this.startTime, this.endTime, (note) => {
      ...
      const animtrack = new CustomEvent(this.startTime).animateTrack(
        "pulseNotes",
        duration,
      );
      animtrack.animate.length = duration;
      for (let i = 0; i < duration; i++) {
        animtrack.animate.add("scale", [[pulseSize, pulseSize, pulseSize, i], [
          1,
          1,
          1,
          i + 0.999,
    ...
    if (pulseSize < 0) {
      MKLog(
        "Objects with negaitve scale have unusual behaviour, set pulseSize to a positive number...",
        "Warning",
      );
    }
  }
  ...
  noteFlyInLR(
    spawnDistance = 10,
    splitBy: "type" | "position" = "position",
    animationEnd = 0.4,
  ) {
    allBetween(this.startTime, this.endTime, (note) => {
      if (splitBy == "type") {
        note.animation.offsetPosition = [[
          spawnDistance * 2 * (note.type - 0.5),
          0,
          0,
          0,
        ...
        note.animation.offsetPosition = [[
          spawnDistance * (Math.abs(note.x - 1.5) / (note.x - 1.5)),
          0,
          0,
          0,
        ], [0, 0, 0, animationEnd, "easeOutQuad"]];
  ...
  noteDropStream(
    ...
    for (let i = 0; i < (this.endTime - this.startTime); i++) {
      ...
      notesBetween(this.startTime, this.endTime, (note) => {
        note.animation.offsetPosition = [[0, streamY - note.y, 0, 0], [
          0,
          streamY - note.y,
          0,
          jumpTime[0],
  ...
  noteSway(
    ...
    for (let i = 0; i < dur; i++) {
      anim.animate.add("offsetPosition", [[0, 0, 0, i * 2, "easeInQuad"], [
        moveDistance,
        0,
        0,
        i * 2 + 0.5,
      ...
      if (rotateFrom == "Top") {
        anim.animate.add("localRotation", [[0, 0, 0, i * 2, "easeInQuad"], [
          0,
          0,
          rotateAngle,
          i * 2 + 0.5,
   ...
   * @param height How high to bounce up. Default - 1.
  ...
  noteBeatBounce(
    height = 1,
    time: [number, number, number] = [0.1, 0.25, 0.4],
    easing?: [EASE, EASE],
  ) {
    allBetween(this.startTime, this.endTime, (note) => {
      if (easing) {
        note.animation.offsetPosition = [[0, 0, 0, time[0]], [
          0,
          height,
          0,
          time[1],
  ...
  noteGhostTrail(
    length = 3,
    gap = 1 / 8,
    fadeTimings: [number, number] = [0, 0.5],
    despawnEasing: EASE = "easeInExpo",
  ) {
    notesBetween(this.startTime, this.endTime, (note) => {
      const nu = copy(note) as Note;
      repeat(length, (i) => {
